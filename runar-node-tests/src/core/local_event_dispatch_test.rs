...
async fn test_local_event_dispatch_multiple_subscribers() -> Result<()> {
    ...
    node.publish("test/event".to_string(), Some(test_data)).await?;
...
async fn test_math_service_plus_external_subscription() -> Result<()> {
    ...
    assert!(received.is_some(), "External subscription should have received the math/added event");
