...
async fn test_diff_remote_subscriptions() -> Result<()> {
    ...
    registry.upsert_remote_peer_subscription(peer, &path_a, "sub_a".into()).await;
    registry.upsert_remote_peer_subscription(peer, &path_b, "sub_b".into()).await;
    ...
    registry.remove_remote_peer_subscription(peer, &path_a).await;
    registry.upsert_remote_peer_subscription(peer, &path_c, "sub_c".into()).await;
