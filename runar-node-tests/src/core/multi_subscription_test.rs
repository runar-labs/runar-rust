...
async fn test_multiple_subscriptions_same_topic() -> Result<()> {
    ...
    let sub_id1 = registry.register_local_event_subscription(
        &topic_path, 
        handler1, 
        EventRegistrationOptions::default()
    ).await?;
    
    let sub_id2 = registry.register_local_event_subscription(
        &topic_path, 
        handler2, 
        EventRegistrationOptions::default()
    ).await?;
    ...
    assert_ne!(sub_ids[0], sub_ids[1], "Subscription IDs should be different");
