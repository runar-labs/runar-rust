...
impl KeyManager {
    ...
    pub fn get_network_key_by_id(&self, network_id: &NetworkId) -> Option<&NetworkKey> {
        self.network_keys.values().find(|nk| *nk.id() == *network_id)
    }
    ...
    pub fn get_or_create_quic_key(&mut self, network_index: u32) -> Result<&QuicKey> {
        ...
        let network_key_id_for_quic_map = self.get_or_create_network_key(network_index)?.id().clone();
        ...
        if !self.quic_keys.contains_key(&network_key_id_for_quic_map) {
            ...
            self.quic_keys.insert(network_key_id_for_quic_map.clone(), quic_key);
        }
        // We can unwrap here because we've just ensured the key exists.
...
mod tests {
    ...
    fn test_get_or_create_quic_key() {
        ...
        let network_id = manager.get_or_create_network_key(network_index).unwrap().id().clone();

        let quic_key1_pk_hex = manager.get_or_create_quic_key(network_index).unwrap().public_key_hex();

        let quic_key2_pk_hex = manager.get_or_create_quic_key(network_index).unwrap().public_key_hex();
        assert_eq!(quic_key1_pk_hex, quic_key2_pk_hex);
        assert_eq!(manager.quic_keys.len(), 1);

        // Retrieve by NetworkId
        let retrieved_key_pk_hex = manager.get_quic_key_by_network_id(&network_id).unwrap().public_key_hex();
        assert_eq!(retrieved_key_pk_hex, quic_key1_pk_hex);

        // QUIC key for a different network
        let network_index_2 = 1;
        let network_id_2 = manager.get_or_create_network_key(network_index_2).unwrap().id().clone();

        let quic_key_diff_network_pk_hex = manager.get_or_create_quic_key(network_index_2).unwrap().public_key_hex();
    ...
    fn test_keys_are_distinct() {
        ...
        let nk_pk_bytes_vec = manager.get_or_create_network_key(index).unwrap().public_key().as_bytes().to_vec();
        let node_pk_bytes_vec = manager.get_or_create_node_key(index).unwrap().public_key().as_bytes().to_vec();
        let quic_pk_bytes_vec = manager.get_or_create_quic_key(index).unwrap().public_key().as_bytes().to_vec();

        assert_ne!(nk_pk_bytes_vec, node_pk_bytes_vec, "Network key and Node key public keys should be different");
        assert_ne!(nk_pk_bytes_vec, quic_pk_bytes_vec, "Network key and QUIC key public keys should be different");
        assert_ne!(node_pk_bytes_vec, quic_pk_bytes_vec, "Node key and QUIC key public keys should be different");
    }
}
