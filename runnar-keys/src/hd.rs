...
pub fn derive_network_key(master_key: &UserMasterKey, network_index: u32) -> Result<NetworkKey> {
    ...
    let (key_bytes, _chain_code) = ed25519_hd_key::derive_from_path(&path_str, master_key.as_bytes());
...
pub fn derive_node_key_from_master_key(master_key: &UserMasterKey, peer_index: u32) -> Result<NodeKey> {
    let path_str = format!(
        "m/{:?}'/{:?}'/{:?}'",
        BIP44_PURPOSE,
        RUNAR_NODE_KEY_COIN_TYPE,
        peer_index
    );
...
mod tests {
    ...
    fn test_derive_network_key_success() {
        ...
        assert_eq!(network_key.keypair.to_bytes(), network_key_again.keypair.to_bytes());
    ...
    fn test_derive_quic_key_from_network_key_success() {
        ...
        assert_eq!(quic_key.public_key().as_bytes(), quic_key_again.public_key().as_bytes());
        assert_eq!(quic_key.keypair.to_bytes(), quic_key_again.keypair.to_bytes());
        ...
        assert_ne!(quic_key.public_key().as_bytes(), quic_key_different_network.public_key().as_bytes());
    }
    ...
    fn test_derive_node_key_from_master_key_success() {
        ...
        assert_eq!(node_key.keypair.to_bytes(), node_key_again.keypair.to_bytes());
        ...
        // Derive with different master key, should be a different key
        let master_key2 = UserMasterKey::generate();
        let node_key_different_master = derive_node_key_from_master_key(&master_key2, peer_index).unwrap();
