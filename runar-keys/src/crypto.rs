...
use rcgen::{CertificateParams, DnType, DistinguishedName, KeyPair as RcgenKeyPair};
...
impl SigningKeyPair {
    ...
    pub fn sign_csr(&self, csr_der: &[u8]) -> Result<Certificate> {
        ...
        if csr.public_key.len() != 32 {
            return Err(KeyError::CertificateError("Invalid Ed25519 public key length in CSR".to_string()));
        }
        ...
        let _csr_verifying_key = VerifyingKey::from_bytes(&public_key_array)
            .map_err(|e| KeyError::CertificateError(format!("Invalid Ed25519 public key in CSR: {}", e)))?;
        ...
        let temp_key_pair = RcgenKeyPair::from_der(temp_p256_key.to_pkcs8_der().unwrap().as_bytes())
            .map_err(|e| KeyError::CertificateError(format!("Failed to create P256 key pair: {}", e)))?;
        ...
        let cert = rcgen::Certificate::from_params(params)
            .map_err(|e| KeyError::CertificateError(format!("Failed to create certificate: {}", e)))?;
        ...
        ca_distinguished_name.push(DnType::CommonName, &format!("ca:{}", hex::encode(self.public_key())));
...
impl Certificate {
    ...
    pub fn validate(&self, ca_verifying_key: &VerifyingKey) -> Result<()> {
        ...
        if not_before_time > not_after_time {
            return Err(KeyError::CertificateError("Certificate has invalid validity period".to_string()));
        }
        ...
        if validity_duration <= 0 {
            return Err(KeyError::CertificateError("Certificate has invalid validity duration".to_string()));
        }
        ...
        if now_timestamp < not_before_time.timestamp() {
            return Err(KeyError::CertificateError("Certificate is not yet valid".to_string()));
        }
        
        if now_timestamp > not_after_time.timestamp() {
            return Err(KeyError::CertificateError("Certificate has expired".to_string()));
        }
        ...
        if signature_alg.algorithm == x509_parser::oid_registry::OID_SIG_ED25519 {
            ...
            if signature_value.len() != 64 {
                return Err(KeyError::CertificateError("Invalid Ed25519 signature length".to_string()));
            }
            ...
            ca_verifying_key.verify_strict(tbs_certificate, &signature)
                .map_err(|e| KeyError::CertificateError(format!("Certificate signature verification failed: {}", e)))?;
            ...
            if signature_value.is_empty() || signature_value.len() < 32 {
                return Err(KeyError::CertificateError("Invalid ECDSA signature".to_string()));
            }
            ...
            return Err(KeyError::CertificateError(
                format!("Unsupported signature algorithm: {:?}", signature_alg.algorithm)
            ));
        }
    ...
    pub fn is_valid(&self) -> bool {
        self.validate(&VerifyingKey::from_bytes(&[0u8; 32]).unwrap()).is_ok()
    }
    ...
    pub fn create_csr(subject_name: &str, signing_key: &SigningKeyPair) -> Result<Vec<u8>> {
        ...
        // Generate the CSR
        let cert = rcgen::Certificate::from_params(params)
            .map_err(|e| KeyError::CertificateError(format!("Failed to create certificate for CSR: {}", e)))?;
    ...
    pub fn parse_csr(csr_bytes: &[u8]) -> Result<CSR> {
        ...
        match x509_parser::certification_request::X509CertificationRequest::from_der(csr_bytes) {
            Ok((_, csr)) => {
                ...
                let subject_name = csr.certification_request_info.subject
                    .iter_common_name()
                    .next()
                    .and_then(|cn| cn.as_str().ok())
                    .unwrap_or("unknown")
                ...
                if subject_name.contains(":ed25519:") {
                    ...
                    if parts.len() == 2 {
                        ...
                        let public_key = hex::decode(public_key_hex)
                            .map_err(|_| KeyError::CertificateError("Invalid Ed25519 public key in CSR subject".to_string()))?;
            ...
            Err(e) => {
                Err(KeyError::CertificateError(format!("Failed to parse PKCS#10 CSR: {}", e)))
            }
        }
    }
}
...
impl EncryptionKeyPair {
    ...
    pub fn encrypt(&self, data: &[u8], recipient_public_key: &[u8]) -> Result<Vec<u8>> {
        let recipient_pk_bytes: [u8; 32] = recipient_public_key
            .try_into()
            .map_err(|_| KeyError::InvalidKeyFormat("Invalid recipient public key length".to_string()))?;
...
mod tests {
    ...
    fn test_encryption_key_operations() {
        ...
        let encrypted = keypair1.encrypt(data, keypair2.public_key().as_bytes()).unwrap();
    ...
    fn test_csr_creation_and_parsing() {
        ...
        assert_eq!(parsed_csr.public_key, hex::decode(hex::encode(keypair.public_key())).unwrap());
    }
}
