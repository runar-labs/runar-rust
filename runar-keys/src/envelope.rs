...
use crate::crypto::{SymmetricEncryption, EncryptionKeyPair, SYMMETRIC_KEY_LENGTH, NONCE_LENGTH};
...
impl Envelope {
    ...
    pub fn new<T: AsRef<[u8]>>(
        ...
        for (recipient_id, recipient_key) in recipients {
            ...
            let (encrypted_key, key_nonce) = SymmetricEncryption::encrypt(&shared_secret, &envelope_key)?;
    ...
    pub fn decrypt(
        ...
        let recipient_key_entry = self.recipient_keys.iter()
            .find(|r| r.recipient_id == recipient_id)
            .ok_or_else(|| KeyError::EnvelopeError(format!(
                "No key found for recipient: {}", recipient_id
            )))?;
