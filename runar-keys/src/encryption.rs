...
pub fn derive_symmetric_key_from_node(node: &NodeKey, context: &[u8]) -> Result<[u8; SYMMETRIC_KEY_LEN]> {
...
pub fn derive_node_shared_key(profile: &UserProfileKey, node: &NodeKey, expires_secs: u64) -> Result<SharedKey> {
    let ikm = [profile.keypair.to_keypair_bytes()[..SECRET_KEY_LENGTH].as_ref(),
               node.verifying_key().as_bytes()].concat();
    let key = hkdf_derive(&ikm, b"runar_node_shared")?;
    Ok(SharedKey::new(key, Some(current_unix_timestamp() + expires_secs)))
}

pub fn derive_network_shared_key(profile: &UserProfileKey, network: &NetworkKey, expires_secs: u64) -> Result<SharedKey> {
    let ikm = [profile.keypair.to_keypair_bytes()[..SECRET_KEY_LENGTH].as_ref(),
               network.public_key().as_bytes()].concat();
...
pub fn encrypt(key: &[u8; SYMMETRIC_KEY_LEN], plaintext: &[u8], aad: Option<&[u8]>) -> Result<(Vec<u8>, [u8; 12])> {
    ...
    let ciphertext = match aad {
        Some(a) => cipher.encrypt(&nonce.into(), Payload { msg: plaintext, aad: a })?,
        None => cipher.encrypt(&nonce.into(), plaintext.as_ref())?,
    };
    Ok((ciphertext, nonce))
}

pub fn decrypt(key: &[u8; SYMMETRIC_KEY_LEN], ciphertext: &[u8], nonce: &[u8; 12], aad: Option<&[u8]>) -> Result<Vec<u8>> {
    let cipher = Aes256Gcm::new_from_slice(key)
        .map_err(|e| crate::error::KeyError::CryptoError(format!("AES init failed: {e}")))?;
    let nonce_ref: &Nonce<aes_gcm::aead::generic_array::typenum::U12> = nonce.into();
    let plain = match aad {
        Some(a) => cipher.decrypt(nonce_ref, Payload { msg: ciphertext, aad: a })?,
