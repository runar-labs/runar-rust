...
impl SetupServer {
    ...
    pub async fn wait_for_certificate(&self) -> Result<NodeCertificateMessage> {
        ...
        self.logger.info(format!("Starting setup server on {}", address));
        ...
        self.logger.info("Setup server started - waiting for mobile device connection...");
                ...
                Ok(socket) => {
                    ...
                    match self.handle_connection(socket).await {
                        Ok(certificate_message) => {
                            self.logger.info("Certificate message received successfully");
                            return Ok(certificate_message);
                        }
                        Err(e) => {
                            self.logger.error(format!("Failed to handle connection: {}", e));
    ...
    async fn handle_connection(&self, socket: TcpStream) -> Result<NodeCertificateMessage> {
        ...
        self.logger.info("Certificate message received and parsed successfully");
        
        Ok(certificate_message)
    }

    async fn read_certificate_message(&self, mut socket: TcpStream) -> Result<NodeCertificateMessage> {
        // Read the message length (4 bytes, big endian)
        let mut length_bytes = [0u8; 4];
        socket.readable().await
            .context("Failed to wait for socket to be readable")?;
        
        socket.try_read(&mut length_bytes)
            .context("Failed to read message length")?;

        let message_length = u32::from_be_bytes(length_bytes) as usize;
        
        self.logger.debug(format!("Reading certificate message of {} bytes", message_length));
        ...
        while bytes_read < message_length {
            ...
            match socket.try_read(&mut message_bytes[bytes_read..]) {
                ...
                Ok(0) => {
                    return Err(anyhow::anyhow!("Connection closed by mobile device"));
                }
                Err(ref e) if e.kind() == std::io::ErrorKind::WouldBlock => {
                    continue;
                }
                Err(e) => {
                    return Err(anyhow::anyhow!("Failed to read certificate message: {}", e));
                }
            }
        }
        ...
        self.logger.debug("Certificate message deserialized successfully");
...
mod tests {
    ...
    async fn test_setup_server() {
        ...
        let logger = Arc::new(Logger::new_root(runar_common::logging::Component::CLI, "test"));
