...
use runar_logging::{Component, Logger, LogLevel, LoggingConfig};
...
impl Keys {
    ...
    pub fn node_decrypt_with_envelope(&self, encrypted_data: Uint8Array) -> Result<Uint8Array> {
        ...
        let encrypted: runar_keys::mobile::EnvelopeEncryptedData = cbor::from_slice(&encrypted_data)
...
impl Transport {
    ...
    pub fn new(keys: &Keys, options: TransportOptions) -> Result<Self> {
            ...
            Box::pin(async move {
                ...
                map.insert(
                    ...
                    serde_cbor::Value::Array(
                        req.payload.profile_public_keys.iter()
        ...
        let event_callback: runar_transporter::transport::EventCallback = Arc::new(move |ev| {
            ...
            Box::pin(async move {
                ...
                map.insert(
                    serde_cbor::Value::Text("profile_public_keys".into()),
                    serde_cbor::Value::Array(
                        ev.payload.profile_public_keys.iter()
                            .map(|k| serde_cbor::Value::Bytes(k.clone()))
                            .collect()
                    ),
                );
        ...
        let peer_connected_callback: runar_transporter::transport::PeerConnectedCallback = Arc::new(move |peer_id, node_info| {
        ...
        let peer_disconnected_callback: runar_transporter::transport::PeerDisconnectedCallback = Arc::new(move |peer_id| {
...
impl CaCreator {
    ...
    pub fn create_issuing_ca(
        ...
        use runar_keys::certificate::{CertificateAuthority, EcdsaKeyPair, CertificateRequest};
        ...
        let issuing_ca_cert = root_ca_inner
            .sign_ca_certificate_request_with_serial(&issuing_ca_csr_der, validity_days, Some(serial as u64))
    ...
    pub fn get_ea_public_key(ea_private_key_der: Uint8Array) -> Result<Uint8Array> {
        let ea_key = runar_keys::certificate::EcdsaKeyPair::from_pkcs8_der(&ea_private_key_der.to_vec())
...
impl CaNode {
    ...
    pub async fn get_issuing_ca_certificate(&self) -> Result<Uint8Array> {
        ...
        let result = RT
            ...
            .await
            .map_err(|e| Error::from_reason(format!("Failed to get issuing CA certificate: {e}")))?;

        let cert_bytes = result.map_err(|e| Error::from_reason(format!("Failed to get certificate bytes: {e}")))?;
        Ok(Uint8Array::from(cert_bytes))
    }
...
pub fn set_log_level(level: i32) -> Result<()> {
    let log_level = match level {
        ...
        5 => LogLevel::Trace,
        _ => {
            return Err(Error::from_reason(format!("Invalid log level: {level}. Must be 0-5")));
        }
    };
