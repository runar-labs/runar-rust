...
fn test_label_resolver_config_validation() {
    ...
    assert!(runar_serializer::traits::ConfigurableLabelResolver::validate_label_config(&empty_config).is_err());

    // Test invalid label with neither network key nor user spec (should fail)
    let invalid_config = LabelResolverConfig {
        label_mappings: std::collections::HashMap::from([
            ("invalid".to_string(), LabelValue {
                network_public_key: None,
                user_key_spec: None,
            }),
        ]),
    };
    assert!(runar_serializer::traits::ConfigurableLabelResolver::validate_label_config(&invalid_config).is_err());

    // Test valid user-only label
    let valid_user_only_config = LabelResolverConfig {
        label_mappings: std::collections::HashMap::from([
            ("user_only".to_string(), LabelValue {
                network_public_key: None,
                user_key_spec: Some(LabelKeyword::CurrentUser),
            }),
        ]),
    };
    assert!(runar_serializer::traits::ConfigurableLabelResolver::validate_label_config(&valid_user_only_config).is_ok());
}
