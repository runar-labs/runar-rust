...
fn test_primitive_types() {
    ...
    let i8_val = ArcValue::new_primitive(42i8);
    assert_eq!(i8_val.to_json().unwrap(), json!(42));
    assert_eq!(i8_val.as_type::<i8>().unwrap(), 42);

    let i16_val = ArcValue::new_primitive(42i16);
    assert_eq!(i16_val.to_json().unwrap(), json!(42));
    assert_eq!(i16_val.as_type::<i16>().unwrap(), 42);

    let i32_val = ArcValue::new_primitive(42i32);
    assert_eq!(i32_val.to_json().unwrap(), json!(42));
    assert_eq!(i32_val.as_type::<i32>().unwrap(), 42);

    let i64_val = ArcValue::new_primitive(42i64);
    assert_eq!(i64_val.to_json().unwrap(), json!(42));
    assert_eq!(i64_val.as_type::<i64>().unwrap(), 42);

    let i128_val = ArcValue::new_primitive(42i128);
    assert_eq!(i128_val.to_json().unwrap(), json!("42"));
    assert_eq!(i128_val.as_type::<i128>().unwrap(), 42);

    // Test all unsigned integer types
    let u8_val = ArcValue::new_primitive(42u8);
    assert_eq!(u8_val.to_json().unwrap(), json!(42));
    assert_eq!(u8_val.as_type::<u8>().unwrap(), 42);

    let u16_val = ArcValue::new_primitive(42u16);
    assert_eq!(u16_val.to_json().unwrap(), json!(42));
    assert_eq!(u16_val.as_type::<u16>().unwrap(), 42);

    let u32_val = ArcValue::new_primitive(42u32);
    assert_eq!(u32_val.to_json().unwrap(), json!(42));
    assert_eq!(u32_val.as_type::<u32>().unwrap(), 42);

    let u64_val = ArcValue::new_primitive(42u64);
    assert_eq!(u64_val.to_json().unwrap(), json!(42));
    assert_eq!(u64_val.as_type::<u64>().unwrap(), 42);

    let u128_val = ArcValue::new_primitive(42u128);
    assert_eq!(u128_val.to_json().unwrap(), json!("42"));
    assert_eq!(u128_val.as_type::<u128>().unwrap(), 42);

    // Test floating point types
    let f32_val = ArcValue::new_primitive(PI);
    // f32 to JSON conversion may have precision differences, so we check the value directly
    let f32_json = f32_val.to_json().unwrap();
    assert!(f32_json.is_number());
    assert_eq!(f32_val.as_type::<f32>().unwrap(), PI);

    let f64_val = ArcValue::new_primitive(std::f64::consts::PI);
    assert_eq!(f64_val.to_json().unwrap(), json!(std::f64::consts::PI));
    assert_eq!(f64_val.as_type::<f64>().unwrap(), std::f64::consts::PI);
    ...
    // Test char
    let char_val = ArcValue::new_primitive('A');
    assert_eq!(char_val.to_json().unwrap(), json!("A"));
    assert_eq!(char_val.as_type::<char>().unwrap(), 'A');

    // Test bytes (should be base64 encoded)
    let bytes_val = ArcValue::new_primitive(b"hello world".to_vec());
    let json_result = bytes_val.to_json().unwrap();
    assert_eq!(json_result, json!("aGVsbG8gd29ybGQ=")); // base64 of "hello world"
    assert_eq!(
        bytes_val.as_type::<Vec<u8>>().unwrap(),
...
fn test_primitive_types_deserialization() {
    // Test basic types that work with the current registry
    let i64_val = ArcValue::new_primitive(42i64);
    let serialized = i64_val.serialize(None).unwrap();
    let i64_val_deser = ArcValue::deserialize(&serialized, None).unwrap();
    assert_eq!(i64_val_deser.to_json().unwrap(), json!(42));
    assert_eq!(i64_val_deser.as_type::<i64>().unwrap(), 42);

    let f64_val = ArcValue::new_primitive(std::f64::consts::PI);
    let serialized = f64_val.serialize(None).unwrap();
    let f64_val_deser = ArcValue::deserialize(&serialized, None).unwrap();
    assert_eq!(
        f64_val_deser.to_json().unwrap(),
        json!(std::f64::consts::PI)
    );
    assert_eq!(
        f64_val_deser.as_type::<f64>().unwrap(),
        std::f64::consts::PI
    );
...
fn test_list_primitives() {
    ...
    let deserialized_list: Vec<String> = deserialized.as_type().unwrap();

    assert_eq!(deserialized_list.len(), 9);
    assert_eq!(deserialized_list[0], "apple");
    assert_eq!(deserialized_list[1], "banana");
    assert_eq!(deserialized_list[2], "cherry");
    assert_eq!(deserialized_list[3], "date");
    assert_eq!(deserialized_list[4], "elderberry");
    assert_eq!(deserialized_list[5], "fig");
    assert_eq!(deserialized_list[6], "grape");
    assert_eq!(deserialized_list[7], "honeydew");
    assert_eq!(deserialized_list[8], "kiwi");
