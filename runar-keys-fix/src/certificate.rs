...
use rcgen::{CertificateParams, Certificate as RcgenCertificate, KeyPair};
use x509_parser::prelude::*;
use rustls_pki_types::{CertificateDer, PrivateKeyDer};

// OpenSSL for proper CA operations
use openssl::x509::{X509Req, X509Builder, X509NameBuilder, X509Extension};
...
use p256::ecdsa::{SigningKey, VerifyingKey, signature::Verifier, Signature};
use p256::EncodedPoint;
use pkcs8::{EncodePrivateKey, DecodePrivateKey, EncodePublicKey};

use crate::error::{Result, KeyError};
...
impl EcdsaKeyPair {
    ...
    pub fn public_key_der(&self) -> Result<Vec<u8>> {
        self.verifying_key.to_public_key_der()
            .map(|der| der.as_bytes().to_vec())
...
impl Serialize for EcdsaKeyPair {
    ...
    {
        let private_key_der = self.private_key_der()
            .map_err(|e| serde::ser::Error::custom(format!("Failed to serialize private key: {}", e)))?;
...
impl<'de> Deserialize<'de> for EcdsaKeyPair {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        let signing_key = SigningKey::from_pkcs8_der(&private_key_der)
            .map_err(|e| serde::de::Error::custom(format!("Failed to deserialize private key: {}", e)))?;
...
impl X509Certificate {
    ...
    pub fn public_key(&self) -> Result<VerifyingKey> {
        ...
        // Extract coordinates
        let x_bytes: [u8; 32] = public_key_bytes[1..33].try_into()
            .map_err(|_| KeyError::InvalidKeyFormat("Invalid X coordinate".to_string()))?;
        let y_bytes: [u8; 32] = public_key_bytes[33..65].try_into()
    ...
    pub fn validate(&self, ca_public_key: &VerifyingKey) -> Result<()> {
        ...
        if now < not_before {
            return Err(KeyError::CertificateValidationError("Certificate not yet valid".to_string()));
        }
        
        if now > not_after {
            return Err(KeyError::CertificateValidationError("Certificate expired".to_string()));
        }
        ...
        // Verify signature
        ca_public_key.verify(tbs_certificate, &signature)
...
impl<'de> Deserialize<'de> for X509Certificate {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        Self::from_der(der_bytes)
            .map_err(|e| serde::de::Error::custom(format!("Failed to deserialize certificate: {}", e)))
    }
}
...
impl CertificateAuthority {
    ...
    pub fn sign_certificate_request(&self, csr_der: &[u8], validity_days: u32) -> Result<X509Certificate> {
        ...
        let req_public_key = req.public_key()
            .map_err(|e| KeyError::CertificateError(format!("Failed to extract public key from CSR: {}", e)))?;
        ...
        let mut cert_builder = X509Builder::new()
            .map_err(|e| KeyError::CertificateError(format!("Failed to create certificate builder: {}", e)))?;
        ...
        let not_before = openssl::asn1::Asn1Time::days_from_now(0)
            .map_err(|e| KeyError::CertificateError(format!("Failed to create not_before time: {}", e)))?;
        let not_after = openssl::asn1::Asn1Time::days_from_now(validity_days)
            .map_err(|e| KeyError::CertificateError(format!("Failed to create not_after time: {}", e)))?;
        ...
        let serial_number = {
            ...
            bn.rand(64, MsbOption::MAYBE_ZERO, false)
                .map_err(|e| KeyError::CertificateError(format!("Failed to generate random serial: {}", e)))?;
            bn.to_asn1_integer()
                .map_err(|e| KeyError::CertificateError(format!("Failed to convert serial to ASN1: {}", e)))?
        };
        ...
        cert_builder.append_extension(
            X509Extension::new_nid(None, None, Nid::KEY_USAGE, "digitalSignature,keyEncipherment")
                .map_err(|e| KeyError::CertificateError(format!("Failed to create key usage extension: {}", e)))?
        ).map_err(|e| KeyError::CertificateError(format!("Failed to add key usage extension: {}", e)))?;
        
        cert_builder.append_extension(
            X509Extension::new_nid(None, None, Nid::EXT_KEY_USAGE, "serverAuth,clientAuth")
                .map_err(|e| KeyError::CertificateError(format!("Failed to create extended key usage extension: {}", e)))?
        ).map_err(|e| KeyError::CertificateError(format!("Failed to add extended key usage extension: {}", e)))?;
    ...
    fn create_self_signed_certificate(key_pair: &EcdsaKeyPair, _subject: &str) -> Result<X509Certificate> {
        let mut params = CertificateParams::new(vec![]);
        
        params.distinguished_name = rcgen::DistinguishedName::new();
        params.distinguished_name.push(rcgen::DnType::CommonName, "Runar User CA");
        params.distinguished_name.push(rcgen::DnType::OrganizationName, "Runar");
