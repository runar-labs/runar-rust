...
async fn test_witness_management() -> Result<()> {
    ...
    let user_icp_attached_sig = AttachedSignaturePrefix::new(
        SelfSigning::Ed25519Sha512,
        user_icp_signature,
        0
    );
    
    let user_signed_icp = SignedEventMessage::new(&user_icp_event, vec![user_icp_attached_sig], None);
    ...
    let witness_icp_attached_sig = AttachedSignaturePrefix::new(
        SelfSigning::Ed25519Sha512,
        witness_icp_signature,
        0
    );
    
    let witness_signed_icp = SignedEventMessage::new(&witness_icp_event, vec![witness_icp_attached_sig], None);
    ...
    let add_witness_attached_sig = AttachedSignaturePrefix::new(
        SelfSigning::Ed25519Sha512,
        add_witness_signature,
        0
    );
    
    let add_witness_signed_event = SignedEventMessage::new(
        &add_witness_event, 
        vec![add_witness_attached_sig], 
        None
    );
    ...
    assert_eq!(updated_state.sn, initial_state.sn + 1, "Sequence number should increase by 1");
    ...
    assert_ne!(updated_state.last_event_digest, initial_state.last_event_digest, "Event digest should change after processing");
    ...
    let remove_witness_attached_sig = AttachedSignaturePrefix::new(
        SelfSigning::Ed25519Sha512,
        remove_witness_signature,
        0
    );
    ...
    assert_eq!(final_state.sn, updated_state.sn + 1, "Sequence number should increase by 1");
    
    // Print the witness list for debugging
    println!("Witnesses after removal: {:?}", final_state.witnesses);
    
    // Check if the last event digest has changed
    assert_ne!(final_state.last_event_digest, updated_state.last_event_digest, "Event digest should change after processing");
