...
async fn test_key_rotation() -> Result<()> {
    ...
    let icp_attached_sig =
        AttachedSignaturePrefix::new(SelfSigning::Ed25519Sha512, icp_signature, 0);
    ...
    let rot_attached_sig =
        AttachedSignaturePrefix::new(SelfSigning::Ed25519Sha512, rot_signature, 0);
    ...
    assert_ne!(
        updated_state.current.public_keys[0], initial_state.current.public_keys[0],
        "Current public key should change after rotation"
    );

    // Verify that the next key digest has been updated
    assert!(
        updated_state.current.threshold_key_digest.is_some(),
        "Next key digest should be present"
    );
