...
use runar_node::services::service_registry::EventHandler;
use runar_node::services::{EventRegistrationOptions, NodeDelegate};
use runar_node::{ServiceRegistry, TopicPath};
...
async fn test_multiple_subscriptions_same_topic() -> Result<()> {
    let logger = Arc::new(Logger::new_root(Component::Custom(
        "test_multiple_subscriptions_same_topic",
    )));
