...
use runar_test_utils::{create_networked_node_test_config, create_node_test_config};
...
async fn test_remote_action_call() -> Result<()> {
    ...
    // let node1_config = create_node_test_config()
    ...
    // let node2_config = create_node_test_config()
    // .expect("Error creating test config")
    //     .with_network_config({
