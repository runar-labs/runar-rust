...
fn test_ca_multiple_creation_and_freeing() {
    ...
    for i in 0..5 {
        ...
        let result = unsafe {
            rn_keys_ca_create_root_ca(cstring_to_ptr(&subject), &mut ca_handle, &mut error)
        };
    ...
    for (i, &ca_handle) in ca_handles.iter().enumerate() {
        ...
        assert!(
            !subject_ptr.is_null(),
            "CA {i} subject should be retrievable"
        );

        let subject_str = unsafe { CStr::from_ptr(subject_ptr).to_string_lossy() };
        assert!(
            subject_str.contains(&format!("CN=Test CA {i}")),
...
fn test_ca_memory_allocation_patterns() {
    ...
    for (i, (ptr, len, is_subject)) in allocated_resources.iter().enumerate() {
        if *is_subject {
            ...
            let subject_str = unsafe { CStr::from_ptr(*ptr as *const c_char).to_string_lossy() };
            assert!(
                subject_str.contains("CN=Test CA"),
                "Subject {i} should still be valid"
            );
        } else {
            ...
            assert_eq!(
                cert_data[0], 0x30,
                "Certificate {i} should still start with sequence tag"
            );
        }
    }
...
fn test_ca_hierarchy_memory_management() {
    ...
    for i in 0..3 {
        ...
        assert!(
            ...
            "Issuing CA {i} handle should not be null"
        );
    ...
    for (i, &issuing_ca_handle) in issuing_ca_handles.iter().enumerate() {
        ...
        assert_eq!(
            result, 0,
            "Issuing CA {i} certificate retrieval should succeed"
        );
...
fn test_ca_free_different_orders() {
    ...
    let mut cert_len: usize = 0;
    let result = unsafe {
        rn_keys_ca_get_certificate_der(issuing_ca_handle, &mut cert_ptr, &mut cert_len, &mut error)
    };
    assert_eq!(
        result, 0,
        "Child CA should still be usable after parent freed"
    );
